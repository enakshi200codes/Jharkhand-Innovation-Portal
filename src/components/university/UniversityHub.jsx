...
export default function UniversityHub({ initialTab = 'overview', userRole = 'university' }) {
  ...
  const visibleTabs = [
    ...
    (userRole === 'university' || userRole === 'government') && { id: 'endorsements', label: 'Student Research Endorsements' },
    (userRole === 'industry' || userRole === 'university' || userRole === 'government') && { id: 'industry', label: 'Industry R&D & MoUs' },
    (userRole === 'community' || userRole === 'university' || userRole === 'government') && { id: 'community', label: 'Grassroots & SIEG Desk' },
  ].filter(Boolean);

  return (
    <div className="bg-[#f4f7f0] min-h-screen pt-6 pb-16 px-4 md:px-6 lg:px-12 space-y-8 text-slate-800">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase text-emerald-800 tracking-wider">
            <span className="bg-emerald-100 px-2 py-0.5 rounded-md">VERIFIED INSTITUTIONAL ENTITY</span>
            <span>• AISHE U-0202</span>
            <span>• BIT MESRA</span>
          </div>
          <h1 className="text-xl md:text-2xl lg:text-3xl font-extrabold text-slate-900 mt-1">
      ...
      {/* DYNAMIC ROLE-BASED SUB-NAVIGATION TAB BAR */}
      <div className="max-w-7xl mx-auto flex gap-2 border-b border-slate-200 pb-3 overflow-x-auto scrollbar-none">
        {visibleTabs.map((tab) => (
          <button 
            key={tab.id}
            type="button"
            ...
            className={`px-4 py-2 text-xs font-bold rounded-xl transition cursor-pointer whitespace-nowrap ${
              activeTab === tab.id ? 'bg-emerald-800 text-white' : 'bg-white text-slate-600 hover:bg-slate-100'
            }`}
          >
            {tab.label}
          </button>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-4">
                    ...
                    <div className="flex items-center justify-between pt-2 flex-wrap gap-2">
                      ...
                      <button className="px-4 py-2 bg-emerald-100 text-emerald-900 font-extrabold text-xs rounded-xl hover:bg-emerald-200 transition cursor-pointer">
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-4">
                    ...
                    <div className="bg-emerald-50 p-4 rounded-2xl flex items-center justify-between flex-wrap gap-2">
                      <div>
                ...
                <div className="space-y-6">
                  <div className="bg-emerald-900 text-white p-6 rounded-3xl space-y-4">
                    ...
                    <button className="w-full py-2.5 bg-white text-emerald-900 font-extrabold text-xs rounded-xl hover:bg-slate-100 transition mt-2 cursor-pointer">
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 space-y-3">
                    ...
                    <div className="space-y-2">
                      <button className="w-full text-left p-2.5 bg-slate-50 hover:bg-slate-100 rounded-xl text-xs font-bold text-slate-700 flex justify-between items-center transition cursor-pointer">
